...
static int64_t xrayfs_unified_pathlookup(const char* path)
{
...
{
    int64_t inode_num = xrayfs_unified_pathlookup(path);
    uint64_t field;
...
{
    uint64_t inode_num = xrayfs_unified_pathlookup(path);
    uint8_t** list;
    size_t len = 0;
    ...
    if (xrayfs_unified_getattr(path, &st))
        return -ENOENT;
...
int main(int argc, char* argv[])
{
    ...
    return fuse_main(argc, argv, &xrayfs_unified_oper, NULL);
}
